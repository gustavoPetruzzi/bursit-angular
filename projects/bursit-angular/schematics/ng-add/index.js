...
function ngAdd(options) {
    return (tree, context) => {
        ...
        if (!configPath || !configBuffer) {
            throw new Error('Could not find angular.json or workspace.json. ' +
        ...
        for (const projectName of Object.keys(projects)) {
            ...
            if (!buildTarget) {
                context.logger.info(`Skipping project "${projectName}" — no build target found.`);
                continue;
            }
            ...
            if (excludedIndex !== -1) {
                ...
                modified = true;
                context.logger.info(`Removing "${excludedPath}" from styles for project "${projectName}".`);
            }
            if (!styles.includes(targetStylePath)) {
                styles.push(targetStylePath);
                modified = true;
                context.logger.info(`Adding "${targetStylePath}" to styles for project "${projectName}".`);
            }
            else {
                context.logger.info(`Project "${projectName}" already includes "${targetStylePath}".`);
            }
        }
        if (modified) {
